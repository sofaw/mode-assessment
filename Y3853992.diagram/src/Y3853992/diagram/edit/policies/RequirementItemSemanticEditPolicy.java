...
import Y3853992.diagram.edit.commands.RequirementConflictsCreateCommand;
import Y3853992.diagram.edit.commands.RequirementConflictsReorientCommand;
	...
	*/
	protected Command getDestroyElementCommand(DestroyElementRequest req) {
		...
		for (Iterator<?> it = view.getTargetEdges().iterator(); it.hasNext();) {
			...
			if (Y3853992VisualIDRegistry.getVisualID(incomingLink) == RequirementConflictsEditPart.VISUAL_ID) {
				DestroyReferenceRequest r = new DestroyReferenceRequest(incomingLink.getSource().getElement(), null,
						incomingLink.getTarget().getElement(), false);
				cmd.add(new DestroyReferenceCommand(r));
				cmd.add(new DeleteCommand(getEditingDomain(), incomingLink));
		...
		for (Iterator<?> it = view.getSourceEdges().iterator(); it.hasNext();) {
			...
			if (Y3853992VisualIDRegistry.getVisualID(outgoingLink) == RequirementConflictsEditPart.VISUAL_ID) {
				DestroyReferenceRequest r = new DestroyReferenceRequest(outgoingLink.getSource().getElement(), null,
						outgoingLink.getTarget().getElement(), false);
				cmd.add(new DestroyReferenceCommand(r));
				cmd.add(new DeleteCommand(getEditingDomain(), outgoingLink));
	...
	protected Command getStartCreateRelationshipCommand(CreateRelationshipRequest req) {
		...
		if (Y3853992ElementTypes.RequirementConflicts_4007 == req.getElementType()) {
			return getGEFWrapper(new RequirementConflictsCreateCommand(req, req.getSource(), req.getTarget()));
		}
	...
	protected Command getReorientReferenceRelationshipCommand(ReorientReferenceRelationshipRequest req) {
		...
		case RequirementConflictsEditPart.VISUAL_ID:
			return getGEFWrapper(new RequirementConflictsReorientCommand(req));
